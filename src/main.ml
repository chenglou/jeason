...
let () =
  ...
  let (ast,_) =
    Parser_flow.program_file ~fail:false ~parse_options "let asd = 1" None in
  let (_,b,_) = ast in
  match b with
           ...
           let open Parser_flow.Ast.Statement in
             (match b with
              ...
              | Parser_flow.Ast.Statement.Debugger  ->
                  print_endline "Debugger"
              | Parser_flow.Ast.Statement.FunctionDeclaration _ ->
                  print_endline "FunctionDeclaration"
              | ((Parser_flow.Ast.Statement.VariableDeclaration
                  ({ VariableDeclaration.kind = kind; declarations }))
                  [@explicit_arity ]) ->
                  (match kind with
                        ...
                        output_value stdout
                          [Str.value Nonrecursive
                             [{
                                ...
                                pvb_expr =
                                  (Exp.constant
                                     ((Const_string ("asd", None))[@implicit_arity
                                                                    ]));
                                pvb_attributes = [];
              ...
              | Parser_flow.Ast.Statement.ExportDeclaration _ ->
                  print_endline "ExportDeclaration"
              | Parser_flow.Ast.Statement.ImportDeclaration _ ->
                  print_endline "ImportDeclaration"))