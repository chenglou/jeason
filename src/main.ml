...
let () =
  ...
  let (ast, _) = Parser_flow.program_file ~fail:false ~parse_options "let asd = console.log('fuck')" None in
  let (_, b, _) = ast in
  (match b with
  ...
  | a :: _ -> (match a with
    | (_, b) ->
      ...
      (* | Parser_flow.Ast.Statement.VariableDeclaration {VariableDeclaration.declarations; kind} -> *)
      | Parser_flow.Ast.Statement.VariableDeclaration {VariableDeclaration.kind; _} ->
        (match kind with
        | VariableDeclaration.Let -> (print_endline "letletletl")
      ...
      | Parser_flow.Ast.Statement.DeclareExportDeclaration _ -> print_endline "DeclareExportDeclaration"
